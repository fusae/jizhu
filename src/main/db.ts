...
export function updateTask(id: string, data: TaskUpdate): Task | undefined {
  ...
  if (shouldResetReminderFlags && data.notified_1h === undefined) { sets.push('notified_1h = ?'); values.push(0); }
  if (shouldResetReminderFlags && data.notified_15m === undefined) { sets.push('notified_15m = ?'); values.push(0); }
  if (shouldResetReminderFlags && data.notified_deadline === undefined) { sets.push('notified_deadline = ?'); values.push(0); }
